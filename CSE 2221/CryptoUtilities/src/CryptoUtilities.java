...
public final class CryptoUtilities {
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        boolean flag = w.canConvertToInt() && w.toInt() == 1;

        if (!flag) {
            copyN.decrement();
            NaturalNumber temp = new NaturalNumber2(copyN);
            powerMod(w, temp, n);
            flag = w.canConvertToInt() && w.toInt() != 1;
        }
        w.copyFrom(copyW);
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        if (n.compareTo(three) <= 0) {
            ...
            int i = 0;
            while (i < NaturalNumber.RADIX) {
                NaturalNumber temp = randomNumber(copyN);
                while (temp.compareTo(one) <= 0 || temp.compareTo(copyN) >= 0) {
                    temp = randomNumber(copyN);
                }
